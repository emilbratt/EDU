...
class Part2
{
    ...
    private static int Dijkstra(int[,] graph, (int row, int col) start, (int row, int col) target)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            if (target.row == state.row && target.col == state.col) Console.WriteLine($"{state} - {current_item.dist}");
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (current_item.direction_index != -1) {
                    if (current_item.dir_count < 4 && current_item.direction_index != i) continue;
                }
                if (current_item.dir_count >= 10 && current_item.direction_index == i) continue;
                Console.WriteLine(current_item.dir_count);
